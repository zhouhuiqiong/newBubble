...
+function ($) {
    ...
    $.confirm = function (text, title, callbackOk, callbackCancel,okText) {
        //okText 自定义
        if (typeof title === 'function') {
            callbackCancel = arguments[2];
            callbackOk =  arguments[1];
            title = undefined;
            okText = arguments[arguments.length - 1];
        };
        return $.modal({
            text: text || '',
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
            buttons: [
                ...
                {text: okText ? okText : defaults.modalButtonOk, bold: true, onClick: callbackOk}
            ]
        });
    };
